...
class CRM_Reports_Page_ListeVisiteursPersonnesAges extends CRM_Core_Page {

  public function run() {
    // Example: Set the page-title dynamically; alternatively, declare a static title in xml/Menu/*.xml
    CRM_Utils_System::setTitle(E::ts('Visiteurs - liste des personnes âgés de plus de 75 ans'));

	// Assign variables for use in a template
		// Liste des Personnes âgés de plus de 75 ans
		$this->assign('PersonnesAges', $this->getPersonnesAgesTable());

    parent::run();
  }

// Création de la table des personnes de plus de 75 ans
	private function getPersonnesAgesTable() {
		$t = [];
		...
		WHERE (
			...
			AND ((YEAR(c.birth_date)) <= (YEAR(NOW()) - 75))
